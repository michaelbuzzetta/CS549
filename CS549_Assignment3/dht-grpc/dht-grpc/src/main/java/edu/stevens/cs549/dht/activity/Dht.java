...
public class Dht extends DhtBase implements IDhtService, IDhtNode, IDhtBackground {
	...
	public OptNodeBindings notify(NodeBindings predDb) {
		...
		if (!pred .hasNodeInfo() || inInterval(cand.getId(), pred.getNodeInfo().getId(), info.getId(), false)) {
			...
			synchronized (state) {
				 ...
				 */
				for (int i = 0; i < db.getBindingsCount(); i++) {
					String k = db.getBindings(i).getKey();
					state.getBroadcaster().broadcastMovedBinding(k);
				}
	...
	public void add(NodeInfo n, String k, String v) throws Failed {
		...
		if (isEqual(n, getNodeInfo())) {
			 ...
			 */
//			client.addBinding(n,
//					edu.stevens.cs549.dht.rpc.Binding.newBuilder()
//							.setKey(k)
//							.setValue(v)
//							.build());
...
//							.build());
	...
	public void join(String host, int port) throws Failed, Invalid {
		 ...
		 * from us.
		 */
		NodeInfo contact = NodeInfo.newBuilder().setHost(host).setPort(port).build();
		succ = client.findSuccessor(contact, info.getId());
		state.clear();
		setSucc(succ);
		stabilize();
		...
		try {
			Log.debug(TAG, "Running fixFingers() after join.");
			fixFingers(5);
		} catch (Exception e) {
			warning("Failed to fix fingers after join: " + e.getMessage());
		}
	}
	...
	public void listenOn(int listenerId, String key, EventProducer eventProducer) {
		...
		if (routing.getPred() == null ||
			...
			if (!routing.getPred().hasNodeInfo() && getNodeInfo().equals(getSucc())) {
				Log.debug(TAG, String.format("Single-node fallback: adding listener for key %s", key));
				state.getBroadcaster().addListener(listenerId, key, eventProducer);
			} else {
	...
	public void listenOff(int listenerId, String key) {
		...
		try {
			...
			Log.debug(TAG, String.format("Skipping removal of non-existent listener %d for key %s", listenerId, key));
		}
	...
	public void startListening(String key, IBindingEventListener bindingEventListener) throws DhtBase.Failed {
		...
		if (state.startListening(key, target)) {
			...
			Log.debug(TAG, String.format("MY INFO IS LISTED RIGHT HERE: %s", myInfo.getId()));
			...
			IEventListener eventListener = new IEventListener() {
				...
				public void onNewBinding(String key, String value) {
					...
					System.out.print("New binding Event, it exists now: " + key + "->" + value);
				...
				public void onMovedBinding(String key) {
					...
					try {
						...
						OptNodeInfo pred = getPred();
						boolean ownsKey = (keyId == local.getId()) ||
								(pred.hasNodeInfo() && DhtBase.inInterval(keyId, pred.getNodeInfo().getId(), local.getId(), true));
						NodeInfo currentTarget = state.getListeningTarget(key);

						if (!ownsKey) {
							NodeInfo newTarget = findSuccessor(keyId);
							if (!newTarget.equals(currentTarget)) {
								System.out.println("New Binding Event, " + key + " moved");
								transferListener(key, bindingEventListener);
							} else {
								Log.debug(TAG, String.format("Skipping transfer for %s: already listening at target %d", key, newTarget.getId()));
							}
						} else {
							Log.debug(TAG, String.format("Suppressing transfer of %s; local node %d owns it", key, local.getId()));
						}

					} catch (Exception e) {
						Log.debug(TAG, String.format("Failed to transfer listener for %s", key));
					}
				}
