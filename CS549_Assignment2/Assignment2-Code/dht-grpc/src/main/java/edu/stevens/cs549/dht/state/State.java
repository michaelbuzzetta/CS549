...
public class State implements IState, IRouting {
	...
	public synchronized NodeInfo closestPrecedingFinger(int id) {
		...
		for (int i = NFINGERS - 1; i >= 0; i--) {
			NodeInfo f = finger[i];
			if (f == null || f.getId() == info.getId()) continue;

			int fingerId = f.getId();
			int nodeId = info.getId();

			boolean between = (nodeId < id && fingerId > nodeId && fingerId < id) ||
					(nodeId > id && (fingerId > nodeId || fingerId < id));
