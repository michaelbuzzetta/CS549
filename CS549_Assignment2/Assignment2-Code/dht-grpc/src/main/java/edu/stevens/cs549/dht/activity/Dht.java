...
public class Dht extends DhtBase implements IDhtService, IDhtNode, IDhtBackground {
	...
	protected NodeInfo closestPrecedingFinger(NodeInfo info, int id) throws Failed {
		...
		if (isEqual(localInfo, info)) {
			...
			if (IRouting.USE_FINGER_TABLE) {
				 ...
				 */
				return getClient().closestPrecedingFinger(info, id);
			} else {
				/*
	...
	public void join(String host, int port) throws Failed, Invalid {
		 ...
		 * from us.
		 */
		NodeInfo knownNode = NodeInfo.newBuilder().setHost(host).setPort(port).build();
		state.clear();
		succ = getClient().findSuccessor(knownNode, info.getId());
		if(succ == null)
		{
			throw new Failed("Join: succ is null. Came from: " + knownNode);
		}
		setSucc(succ);
		stabilize();
